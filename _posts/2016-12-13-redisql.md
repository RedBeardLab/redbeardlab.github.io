...
The very first step will be to add data persitency that we believe to be fundamental even if option for some use cases optional.

Then we would like to move following the necessity of the community, so ideas and use cases are extremely welcome.

We do have a couple of ideas: 

1. Stream all the statements that modify the data, everything but `SELECT`s

2. A cache system to store the result of some complex select
